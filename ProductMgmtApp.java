...
public class ProductMgmtApp {
    ...
    public static void printProducts(Product[] products) {
        Arrays.sort(products, (a, b) -> Double.compare(b.getUnitPrice(), a.getUnitPrice()));
        ...
        for (int i = 0; i < products.length; i++) {
            ...
            System.out.printf(
                    "  {\"productId\": %d, \"name\": \"%s\", \"dateSupplied\": \"%s\", \"quantityInStock\": %d, \"unitPrice\": %.2f}%s\n",
        ...
        for (Product p : products) {
            System.out.printf(
                    "  <product><productId>%d</productId><name>%s</name><dateSupplied>%s</dateSupplied><quantityInStock>%d</quantityInStock><unitPrice>%.2f</unitPrice></product>\n",
                    p.getProductId(), p.getName(), p.getDateSupplied(), p.getQuantityInStock(), p.getUnitPrice());
        }
        System.out.println("</products>");
